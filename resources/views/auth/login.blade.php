...
@section('title', 'Đăng Nhập - ' . ($settings['store_name'] ?? 'VPNStore'))
...
<section style="min-height:calc(100vh - 68px); display:flex; align-items:center; justify-content:center; padding:40px 24px; position:relative; overflow:hidden;">
    ...
    <div style="width:100%; max-width:420px; position:relative; z-index:1;">
        ...
        <div class="card" style="border-color:rgba(124,58,237,0.3); padding:32px;">
            ...
            <form method="POST" action="{{ route('auth.login.post') }}" style="display:flex; flex-direction:column; gap:16px;">
                @csrf
                ...
                <div style="display:flex; flex-direction:column; gap:6px;">
                    ...
                    <div style="position:relative; display:flex; align-items:center;">
                        ...
                        <input type="email" name="email" placeholder="email@gmail.com" value="{{ old('email') }}" required autocomplete="email" autofocus style="background:var(--bg-input); border:1px solid var(--border); color:var(--text-primary); padding:10px 14px 10px 40px; border-radius:var(--radius); font-size:0.85rem; width:100%; outline:none;">
                    </div>
                </div>

                <div style="display:flex; flex-direction:column; gap:6px;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <label style="font-size:0.8rem; font-weight:700; color:var(--text-secondary); margin-bottom:0;">Mật Khẩu</label>
                        <a href="{{ route('auth.forgot-password') }}" style="font-size:0.75rem; color:var(--primary-light); text-decoration:none;">Quên mật khẩu?</a>
                    </div>
                    <div style="position:relative; display:flex; align-items:center;">
                        <span style="position:absolute; left:14px; color:var(--text-muted); font-size:1rem;"><i class="bi bi-lock"></i></span>
                        <input type="password" name="password" id="loginPassword" placeholder="••••••••" required autocomplete="current-password" style="background:var(--bg-input); border:1px solid var(--border); color:var(--text-primary); padding:10px 40px 10px 40px; border-radius:var(--radius); font-size:0.85rem; width:100%; outline:none;">
                        <button type="button" onclick="togglePassword('loginPassword')" style="position:absolute; right:14px; background:none; border:none; color:var(--text-muted); cursor:pointer;"><i class="bi bi-eye" id="eye-icon-loginPassword"></i></button>
                    </div>
                </div>

                <label style="display:flex; align-items:center; gap:8px; cursor:pointer; font-size:0.85rem; color:var(--text-secondary); margin-bottom:8px;">
            ...
            <div style="text-align:center; font-size:0.85rem; color:var(--text-muted); margin-top:24px;">
                Chưa có tài khoản?
                <a href="{{ route('auth.register') }}" style="color:var(--primary-light); font-weight:700; text-decoration:none;">Đăng Ký Ngay</a>
            </div>
        </div>
    </div>
</section>
